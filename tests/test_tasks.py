...
from learning_credentials.tasks import (
    ...
    generate_credentials_for_course_task,
)
...
def test_generate_credential_for_user():
    ...
    with (
        ...
        patch(
            'learning_credentials.tasks.generate_credential_for_user_task',
        ) as mock_task,
    ):
        mock_config = Mock()
        ...
        generate_credential_for_user_task(course_config_id, user_id)
...
def test_generate_credentials_for_course_with_filtering():
    ...
    with (
        patch('learning_credentials.models.CredentialConfiguration.objects.get') as mock_get,
        patch(
            'learning_credentials.tasks.generate_credential_for_user_task.delay',
        ) as mock_delay,
    ):
        mock_config = Mock()
        mock_get.return_value = mock_config
        ...
        generate_credentials_for_course_task(course_config_id)
...
def test_generate_all_credentials():
    """Test if `generate_credentials_for_course_task.delay` is called for each enabled configuration."""
    ...
    with (
        ...
        patch('learning_credentials.tasks.generate_credentials_for_course_task.delay') as mock_delay,
    ):
        generate_all_credentials_task()
