    ...
    **POST**: Triggers credential generation for an eligible user.
    ...
    @apidocs.schema(
        ...
        responses={
            ...
            400: "User is not eligible for this credential.",
            403: "User is not authenticated.",
            404: "Learning context or credential type not found.",
    ...
    def post(self, request: "Request", learning_context_key: str, credential_type_id: int) -> Response:
        ...
        The user must be eligible and must not already have an existing valid credential of this type.

        **Query Parameters**

        - ``username`` (staff only): Trigger generation for a specific user.
        ...
        **Example Response**
        ...
        .. code-block:: json
            ...
            {
              "detail": "Credential generation started."
            }
        """
        username = request.query_params.get('username')
        user = get_object_or_404(User, username=username) if username else request.user
        ...
        existing_credential = (
            Credential.objects.filter(user_id=user.id, configuration=config)
            .exclude(status=Credential.Status.ERROR)
            .first()
        )
        ...
        if existing_credential:
            return Response(
                {"detail": "User already has a credential of this type."},
                status=status.HTTP_409_CONFLICT,
            )

        if not config.get_eligible_user_ids(user_id=user.id):
            return Response(
                {"detail": "User is not eligible for this credential."},
